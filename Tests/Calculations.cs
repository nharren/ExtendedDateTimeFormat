...
namespace Tests
{
    public static class Calculations
    {
        ...
        public static Calculation[] DifferenceCalculations =
        {
            new Calculation(() => (new ExtendedDateTime(1999, 5, 3, 15, 5, 2, TimeSpan.Zero) - new ExtendedDateTime(1963, 6, 1, 23, 50, 55, TimeSpan.Zero)).ToString(), string.Format("Start Date: {0}\nEnd Date: {1}", new ExtendedDateTime(1963, 6, 1, 23, 50, 55, TimeSpan.Zero), new ExtendedDateTime(1999, 5, 3, 15, 5, 2, TimeSpan.Zero))),
            new Calculation(() => (new ExtendedDateTime(1988, 7, 12, 23, 0, 0, TimeSpan.Zero) - new ExtendedDateTime(1934, 11, 30, 13, 49, 35, TimeSpan.Zero)).ToString(), string.Format("Start Date: {0}\nEnd Date: {1}", new ExtendedDateTime(1934, 11, 30, 13, 49, 35, TimeSpan.Zero), new ExtendedDateTime(1988, 7, 12, 23, 0, 0, TimeSpan.Zero)))
        };
    }
}
