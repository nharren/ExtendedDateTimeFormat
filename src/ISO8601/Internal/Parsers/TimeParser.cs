    ...
    internal static class TimeParser
    {
        internal static Time Parse(string input)
        {
            ...
            if (nextChar == '.' || nextChar == ',')
            {
                ...
                {
                    return new Time(double.Parse(hour + "." + new string(fractionPartBuffer.ToArray())));
                }
                else
                {
                    return new Time(double.Parse(hour + "." + new string(fractionPartBuffer.ToArray()))) { UtcOffset = ParseUtcOffset(input, fractionIndex) };
                }
            }
            ...
            if (nextChar == '.' || nextChar == ',')
            {
                ...
                if (fractionIndex == input.Length)
                {
                    return new Time(hour, double.Parse(minute + "." + new string(fractionPartBuffer.ToArray())));
                }
                else
                {
                    return new Time(hour, double.Parse(minute + "." + new string(fractionPartBuffer.ToArray()))) { UtcOffset = ParseUtcOffset(input, fractionIndex) };
                }
            }
            else if (nextChar == '+' || nextChar == '-' || nextChar == 'Z')
            {
            ...
            if (nextChar == '.' || nextChar == ',')
            {
                ...
                if (fractionIndex == input.Length)
                {
                    return new Time(hour, minute, double.Parse(second + "." + new string(fractionPartBuffer.ToArray())));
                }
                else
                {
                    return new Time(hour, minute, double.Parse(second + "." + new string(fractionPartBuffer.ToArray()))) { UtcOffset = ParseUtcOffset(input, fractionIndex) };
                }
            }
            else
            {
